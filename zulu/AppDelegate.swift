...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    lazy var motioinManager = CMMotionManager()
   ...
   // var settings = UserDefaults.standard
        ...
        if settings.string(forKey: Constants.kSortField) == nil
            {
                ...
                settings.set("contactName", forKey: Constants.kSortField)
                settings.set("birthday", forKey: Constants.kSortField)
                settings.set("state", forKey: Constants.kSortField)
                settings.set("zipCode", forKey: Constants.kSortField)
    ...
    /* 
            The persistent container for the application. This implementation creates and retuens a container, having loaded the store for the application to it. This property is optional since there are legitimate error conditions that could cause the creation of the store to fail*/
        
        let container = NSPersistentContainer(name: "MyContantListModel")
    ...
    func saveContext (){
            ...
            if context.hasChanges{
                do{
                        ...
                        //Replace this implementation with code to handle the error approprietely
                    // fatalError() causes the application to generate a crash log and terminate. You should
                    // not use this function in a shipping application, although it may be useful during
                    // development
